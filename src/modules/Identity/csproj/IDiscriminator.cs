...
namespace Fuxion.Identity
{
	...
	public static class DiscriminatorExtensions
	{
		...
		public static bool IsValid(this IDiscriminator me) => !Comparer.AreEquals(me.TypeId, me.TypeId?.GetType().GetDefaultValue())
			...
			switch (mode)
			{
				...
				case PrintMode.Table:
					...
					foreach (var sco in me)
						Printer.WriteLine("│" + sco.TypeId.ToString().PadRight(maxTypeId, ' ') + "│" + sco.TypeName.PadRight(maxTypeName, ' ') + "│" + (sco.Id?.ToString() ?? "null").PadRight(maxId, ' ') + "│" + (sco.Name ?? "null").PadRight(maxName, ' ') + "│");
					Printer.WriteLine("└" + ("".PadRight(maxTypeId, '─')) + "┴" + ("".PadRight(maxTypeName, '─')) + "┴" + ("".PadRight(maxId, '─')) + "┴" + ("".PadRight(maxName, '─')) + "┘");
					break;
			}
		}
