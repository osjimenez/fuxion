...
public class Locker<TObjectLocked> : IDisposable
{
	...
	private Task DelegateReadAsync(Delegate del, params object?[] pars) => TaskManager.StartNew((d, ps) =>
																					 {
																						 _ReaderWriterLockSlim.EnterReadLock();
																						 try
																						 {
	...
	private Task<TResult> DelegateReadAsync<TResult>(Delegate del, params object?[] pars) where TResult : notnull => TaskManager.StartNew<Delegate, object?[], TResult>((d, ps) =>
																															   {
																																   _ReaderWriterLockSlim.EnterReadLock();
																																   try
																																   {
	...
	private Task<TResult?> DelegateReadNullableAsync<TResult>(Delegate del, params object?[] pars) => TaskManager.StartNew<Delegate, object?[], TResult?>((d, ps) =>
																												{
																													_ReaderWriterLockSlim.EnterReadLock();
																													try
																													{
	...
	private Task DelegateWriteAsync(Delegate del, params object?[] pars) => TaskManager.StartNew((d, ps) =>
																					  {
																						  _ReaderWriterLockSlim.EnterWriteLock();
																						  try
																						  {
	...
	private Task<TResult> DelegateWriteAsync<TResult>(Delegate del, params object?[] pars) => TaskManager.StartNew((d, ps) =>
																										{
																											_ReaderWriterLockSlim.EnterWriteLock();
																											try
																											{
