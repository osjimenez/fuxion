...
public abstract class PropertyFallbackResolver
{
	...
	protected void FallbackWriteRaw(object value, Utf8JsonWriter writer, JsonSerializerOptions options, List<PropertyFallbackResolver> resolvers)
	{
		...
		converterType.GetMethod(nameof(FallbackConverter<object>.FallbackWriteRaw))?
			.Invoke(converter, new object?[] { value, writer, options, resolvers });
	}
}
...
public class FallbackConverter<T> : JsonConverter<T>
{
	...
	{
		if (!resolvers.OfType<IfNullWritePropertyFallbackResolver>().Any())
			this.resolvers.Add(new IfNullWritePropertyFallbackResolver());
		if (!resolvers.OfType<IfMemeberInfoWriteNamePropertyFallbackResolver>().Any())
			this.resolvers.Add(new IfMemeberInfoWriteNamePropertyFallbackResolver());
		if (!resolvers.OfType<CollectionPropertyFallbackResolver>().Any())
			this.resolvers.Add(new CollectionPropertyFallbackResolver());
		this.resolvers.AddRange(resolvers);
	}
	private readonly List<PropertyFallbackResolver> resolvers = new();
