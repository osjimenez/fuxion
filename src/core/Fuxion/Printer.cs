...
public class Printer
{
	...
	private static readonly Dictionary<object, PrinterInstance> factories = new Dictionary<object, PrinterInstance>();
...
public class CallResultArg<T> : DisposableEnvelope<T> where T : notnull
{
	...
	private readonly char resultConnectorChar;
...
internal class PrinterInstance : IPrinter
{
	...
	private readonly Locker<Dictionary<int, char>> verticalConnectorLevels = new Locker<Dictionary<int, char>>(new Dictionary<int, char>());
	[DebuggerHidden]
	public Action<string> WriteLineAction { get; set; } = m => Debug.WriteLine(m);

	private readonly List<string> lineMessages = new List<string>();
