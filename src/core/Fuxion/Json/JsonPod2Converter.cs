	...
	where TDiscriminator : notnull
{
	public override TPod? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
	{
		...
		while (reader.Read())
		{
			...
			if (propertyName == nameof(JsonPod2<string, string>.Payload))
			{
	...
	public override void Write(Utf8JsonWriter writer, TPod value, JsonSerializerOptions options)
	{
		...
		{
			if (prop.Name == nameof(JsonPod2<string, string>.Headers))
			{
				if (prop.GetValue(value) is not JsonPodCollection2<TDiscriminator> headers)
					throw new InvalidProgramException($"Property '{prop.Name}' of type '{prop.DeclaringType?.Name}' must be of Type '{nameof(JsonPodCollection2<TDiscriminator>)}'");
