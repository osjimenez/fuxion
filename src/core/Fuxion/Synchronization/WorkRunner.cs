		...
		internal async Task<WorkPreview> PreviewAsync(IPrinter printer)
		{
			...
			using (printer.Indent($"Work '{Definition.Name}'"))
			{
				...
				void SearchComparator(ISideRunner side)
				{
					...
					{
						var mts = master.GetAllItemsType();
					...
					if (cc.Count() != 1) throw new ArgumentException($"One, and only one '{nameof(ISide)}' must be added for master side '{master.Definition.Name}' and each side");
				...
				{
					if (sid.Comparator?.GetItemTypes().typeA == master.GetItemType())
					{
						// Master is A in this comparer
						sid.Results = sid.Comparator?.CompareSides(master, sid, false, printer) ?? new List<IComparatorResultInternal>();
					}
					else
					{
						// Master is B in this comparer
						sid.Results = sid.Comparator?.CompareSides(sid, master, true, printer) ?? new List<IComparatorResultInternal>();
					}
				});
				...
				ICollection<IItemRunner> AnalyzeResults(ICollection<ISideRunner> sides)
				{
					...
					foreach (var gro in sides
						.SelectMany(side => side.Results.Select(result =>
						(
							Key: result.Key,
							MasterRunner: side.SearchMasterSubSide(master),
							MasterItemType: side.SearchMasterSubSide(master).GetItemType(),
							MasterItem: result.MasterItem,
							MasterItemName: side.SearchMasterSubSide(master).GetItemName(result.MasterItem),
							MasterItemTag: side.SearchMasterSubSide(master).GetItemTag(result.MasterItem),
							//SideId = side.Id,
							Side: side,
				...
				foreach (var item in Items)
				{
					...
					foreach (var side in item.SideRunners)
					{
						...
						ICollection<ItemRelationPreview> ProcessSubItems(object parent, ICollection<IItemRunner> items)
						{
							...
							{
								ItemRelationPreview rel = default!;
								if (parent is ItemSidePreview isp)
									rel = new ItemRelationPreview(isp, i.Id);
								else if (parent is ItemRelationPreview irp)
									rel = new ItemRelationPreview(irp, i.Id);
