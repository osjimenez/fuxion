...
public class LicensingManager
{
	...
	static string ToXmlString(RSACryptoServiceProvider rsa, bool includePrivateParameters)
	{
		if (includePrivateParameters)
		{
			var parameters = rsa.ExportParameters(true);
			return string.Format(
				"<RSAKeyValue>\r\t<Modulus>{0}</Modulus>\r\t<Exponent>{1}</Exponent>\r\t<P>{2}</P>\r\t<Q>{3}</Q>\r\t<DP>{4}</DP>\r\t<DQ>{5}</DQ>\r\t<InverseQ>{6}</InverseQ>\r\t<D>{7}</D>\r</RSAKeyValue>",
				Convert.ToBase64String(parameters.Modulus ?? Array.Empty<byte>()), Convert.ToBase64String(parameters.Exponent ?? Array.Empty<byte>()),
				Convert.ToBase64String(parameters.P ?? Array.Empty<byte>()), Convert.ToBase64String(parameters.Q ?? Array.Empty<byte>()), Convert.ToBase64String(parameters.DP ?? Array.Empty<byte>()),
				Convert.ToBase64String(parameters.DQ ?? Array.Empty<byte>()), Convert.ToBase64String(parameters.InverseQ ?? Array.Empty<byte>()),
