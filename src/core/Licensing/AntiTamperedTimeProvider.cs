...
///    - Optional - Receive NTP servers from external source to maintain the list up to date. When no connection with this external source, the last obtained server list must stay persistent and
///    encrypted in local machine
///    PROVIDERS
...
public class AntiTamperedTimeProvider : ITimeProvider
{
	...
	DateTime GetUtc()
	{
		...
		if (reliable != null || antiBack != null) return (reliable ?? antiBack)!.Value;
