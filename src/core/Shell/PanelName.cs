...
public struct PanelName
{
	...
	public static PanelName Parse(string panelName)
	{
		...
		if (index == -1) return new(panelName, null);
		return new(panelName.Substring(0, index), panelName.Substring(index + 1));
	}
	...
	public static bool operator !=(PanelName v1, PanelName v2) => v1.Name != v2.Name && v1.Key == v2.Key;
	public override string ToString() => Name + (Key != null ? KEY_SEPARATOR + Key : "");
}