...
namespace Fuxion.Shell
{
	public class Cache
	{
		...
		public void Add<T>(IEqualityComparer<T> comparer) where T : BaseDvo => caches.GetOrAdd(typeof(T), (new SourceCache<T, Guid>(_ => _.Id), comparer));
		public SourceCache<T, Guid> Get<T>() where T : BaseDvo => (SourceCache<T, Guid>)caches[typeof(T)].SourceCache;
