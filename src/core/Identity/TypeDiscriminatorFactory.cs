...
public class TypeDiscriminatorFactory
{
	...
	void Initialize()
	{
		...
		// Coloco los tipos base y derivados de cada tipo
		{
			...
			foreach (var type in allTypes) SetHierarchy(type);
			// Aplico el DisableMode (sé que todos los disable modes de una entrada son iguales)
			{
				...
				{
					ClearType(ty);
					foreach (var der in ty.DerivedTypes.ToList()) ClearDerived(der);
				}
			...
			foreach (var type in allTypes.Where(t => t.BaseType == null && t.DeepBaseType != null && !t.DeepBaseType.DerivedTypes.Contains(t))) type.DeepBaseType?.DerivedTypes.Add(type);
		}
	...
	List<InvalidTypeDiscriminatorException> ValidateEntries(bool throwException = true)
	{
	...
	List<InvalidTypeDiscriminatorException> ValidateRegistrations(bool throwException = true)
	{
#if DEBUG && TRUE
		foreach (var ent in entries)
		{
		...
		foreach (var ent in entries)
		{
			...
			foreach (var dis in ent.Discriminator.Inclusions.Where(i => !i.Exclusions.Contains(ent.Discriminator)))
				errors.Add(new($"The type discriminator '{ent.Discriminator.Name}' include the type discriminator '{dis.Name}', but '{dis.Name}' not exclude '{ent.Discriminator.Name}'"));
			foreach (var dis in ent.Discriminator.Exclusions.Where(i => !i.Inclusions.Contains(ent.Discriminator)))
				errors.Add(new($"The type discriminator '{ent.Discriminator.Name}' exclude the type discriminator '{dis.Name}', but '{dis.Name}' not include '{ent.Discriminator.Name}'"));
	...
	public TypeDiscriminator? FromId(string id)
	{
		...
		if (all.Count() > 1) throw new InvalidStateException($"More than one discriminator for the id '{id}'. Use '{nameof(AllFromId)}' method instead");
