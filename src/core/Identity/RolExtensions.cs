...
public static class RolExtensions
{
	...
	{
		using (var res = Printer.CallResult<bool>()) return res.Value = me.Can(Functions.Admin).Anything();
	}
	#endregion
	...
	public static bool Anything(this IRolCan me)
	{
		using (var res = Printer.CallResult<bool>())
		{
			...
			foreach (var fun in ime.Functions)
			{
				...
				if (!permissions.Any() || permissions.Any(p => p.Scopes.Any())) return res.Value = false;
			}
	...
	public static bool Something(this IRolCan me)
	{
		using (var res = Printer.CallResult<bool>())
		{
			...
			{
				var permissions = ime.Rol.SearchPermissions(false, fun, TypeDiscriminator.Empty);
				if (!permissions.Any(p => p.Value)) return res.Value = false;
			}
			return res.Value = true;
		}
	}
	...
	{
		using (var res = Printer.CallResult<bool>()) return res.Value = ((IInternalRolCan)me).CheckDiscriminators(true, typeDiscriminator, discriminators);
	}
	public static bool ByAny(this IRolCan me, TypeDiscriminator typeDiscriminator, params IDiscriminator[] discriminators)
	{
		using (var res = Printer.CallResult<bool>()) return res.Value = ((IInternalRolCan)me).CheckDiscriminators(false, typeDiscriminator, discriminators);
	}
	#endregion
	...
	{
		using (var res = Printer.CallResult<bool>()) return res.Value = ((IInternalRolCan)me).CheckDiscriminators(true, Singleton.Get<TypeDiscriminatorFactory>().FromType(type));
	}
	...
	{
		using (var res = Printer.CallResult<bool>()) return res.Value = ((IInternalRolCan)me).CheckInstances(true, values);
	}
	public static bool AnyInstances<T>(this IRolCan me, IEnumerable<T> values)
	{
		using (var res = Printer.CallResult<bool>()) return res.Value = ((IInternalRolCan)me).CheckInstances(false, values);
	}
	// -------------------------- IMPLEMENTATION
	static bool CheckInstances<T>(this IInternalRolCan me, bool forAll, IEnumerable<T> values)
	{
		using (var res = Printer.CallResult<bool>())
		{
			if (me.Rol == null) return res.Value = false;
			bool CheckInstance(T value) => ByAll(me, Singleton.Get<TypeDiscriminatorFactory>().FromType<T>(), typeof(T).GetDiscriminatorsOfDiscriminatedProperties(value).ToArray());
			var r = forAll ? values.Where(value => CheckInstance(value)).Count() == values.Count() : values.Where(value => CheckInstance(value)).Any();
