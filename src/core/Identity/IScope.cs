	...
	public static void Print(this IEnumerable<IScope> me, PrintMode mode)
	{
		switch (mode)
		{
			...
			case PrintMode.Table:
				...
				var nameLength = new[] { "NAME".Length }.Concat(me.Select(p => p.Discriminator.Name?.Length ?? 0)).Max();
				...
				foreach (var sco in me)
					Printer.WriteLine("│" + sco.Discriminator.TypeName.PadRight(typeLength, ' ') + "│" + sco.Discriminator.Name?.PadRight(nameLength, ' ') + "│" + sco.Propagation.ToString().PadRight(propagationLength, ' ') + "│");
				Printer.WriteLine("└" + ("".PadRight(typeLength, '─')) + "┴" + ("".PadRight(nameLength, '─')) + "┴" + ("".PadRight(propagationLength, '─')) + "┘");
				break;
		}
	}
...
{
	private readonly DiscriminatorEqualityComparer disCom = new DiscriminatorEqualityComparer();
