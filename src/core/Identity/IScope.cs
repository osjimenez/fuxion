...
public static class IScope_Extensions
{
	...
	public static IEnumerable<IDiscriminator> AllDiscriminators(this IScope me)
	{
		...
		if (me.Propagation.HasFlag(ScopePropagation.ToExclusions)) res.AddRange(me.Discriminator.GetAllExclusions());
		if (me.Propagation.HasFlag(ScopePropagation.ToMe)) res.Add(me.Discriminator);
		if (me.Propagation.HasFlag(ScopePropagation.ToInclusions)) res.AddRange(me.Discriminator.GetAllInclusions());
		return res.Distinct();
	}
...
public class ScopeEqualityComparer : IEqualityComparer<IScope>
{
	...
	bool AreEquals(object? obj1, object? obj2)
	{
		...
		return disCom.Equals(sco1.Discriminator, sco2.Discriminator) && AreEquals(sco1.Propagation, sco2.Propagation);
	}
}