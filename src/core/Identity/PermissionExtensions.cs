...
{
	public static bool IsValid(this IPermission me) =>
		me.Function != null && (me.Scopes?.All(s => s.Discriminator != null) ?? true) && me.Scopes?.Select(s => s.Discriminator.TypeKey).Distinct().Count() == me.Scopes?.Count();
	...
	internal static bool MatchByDiscriminatorsInclusionsAndExclusions(this IPermission me, bool forFilter, TypeDiscriminator typeDiscriminator, params IDiscriminator[] discriminators)
	{
		...
		using (Printer.Indent($"CALL {nameof(MatchByDiscriminatorsInclusionsAndExclusions)}:", '│'))
		{
			...
			if (typeDiscriminator == null) throw new ArgumentException($"'{nameof(typeDiscriminator)}' cannot be null");
			bool Compute()
			{
				...
				using (Printer.Indent("Checking type discriminator"))
				{
					...
					if (scopeOfTypeOfTypeDiscriminator != null)
					{
						...
						{
							Printer.WriteLine(
								$"The {nameof(typeDiscriminator)} '{typeDiscriminator}' is related to permission scope '{scopeOfTypeOfTypeDiscriminator}' on discriminator '{scopeDiscriminatorRelatedWithTargetDiscriminator}', check discriminators");
	...
	public static string ToOneLineString(this IPermission me) => $"{me.Value} - {me.Function.Name} - {me.Scopes.Count()}";
