...
public static class FunctionExtensions
{
	...
	static IEnumerable<T> GetAllInclusions<T>(this IInclusive<T> me, List<T> progress)
	{
		var res = progress;
		if (me.Inclusions == null) return res;
		var fs = me.Inclusions.Except(res).ToList();
		res.AddRange(fs);
		foreach (var f in fs.Cast<IInclusive<T>>()) GetAllInclusions(f, res);
		return res;
	}
	public static IEnumerable<T> GetAllInclusions<T>(this IInclusive<T> me) => GetAllInclusions(me, new(new T[] { }));
	public static IEnumerable<IDiscriminator> GetAllRelated(this IDiscriminator me, ScopePropagation propagation)
	{
		...
		if (propagation.HasFlag(ScopePropagation.ToMe)) res.Add(me);
		if (propagation.HasFlag(ScopePropagation.ToInclusions)) res.AddRange(me.GetAllInclusions());
		if (propagation.HasFlag(ScopePropagation.ToExclusions)) res.AddRange(me.GetAllExclusions());
		return res;
	}
	static IEnumerable<T> GetAllExclusions<T>(this IExclusive<T> me, List<T> progress)
	{
		var res = progress;
		if (me.Exclusions == null) return res;
		var fs = me.Exclusions.Except(res).ToList();
		res.AddRange(fs);
		foreach (var f in fs.Cast<IExclusive<T>>()) GetAllExclusions(f, res);
		return res;
	}
	public static IEnumerable<T> GetAllExclusions<T>(this IExclusive<T> me) => GetAllExclusions(me, new(new T[] { }));
	public static void Print(this IEnumerable<IFunction> me, PrintMode mode)
	{
		switch (mode)
		{
			case PrintMode.OneLine:
				foreach (var fun in me) Printer.WriteLine(fun.Id + "-" + fun.Name);
				break;
