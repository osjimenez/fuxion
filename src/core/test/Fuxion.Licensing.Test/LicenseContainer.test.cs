...
{
	public LicenseContainerTest(ITestOutputHelper output) => this.output = output;//Container con = new Container();//con.RegisterInstance<ILicenseProvider>(new LicenseProviderMock());//con.RegisterInstance<ILicenseStore>(new LicenseStoreMock());//con.RegisterInstance<IHardwareIdProvider>(new HardwareIdProviderMock());//con.RegisterInstance<ITimeProvider>(new MockTimeProvider());//con.Register<LicensingManager>();//Factory.AddInjector(new SimpleInjectorFactoryInjector(con));
