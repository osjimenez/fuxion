...
namespace Fuxion.Identity.Test
{
    public class ContextTest : BaseTest
    {
        public ContextTest(ITestOutputHelper output) : base(output)
        {
            ...
            // TypeDiscriminators
            c.RegisterInstance(new TypeDiscriminatorFactory().Transform(fac =>
            {
            ...
            c.RegisterInstance<ICurrentUserNameProvider>(new GenericCurrentUserNameProvider(() => Context.Rols.Identity.Root.UserName));
            c.RegisterInstance<IKeyValueRepository<string, IIdentity>>(new IdentityMemoryTestRepository());
