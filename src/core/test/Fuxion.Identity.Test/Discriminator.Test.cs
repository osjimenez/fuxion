...
public class DiscriminatorTest : BaseTest
{
	...
	public void Compare_GuidDiscriminator()
	{
		...
		Assert.True(comparer.Equals(new GuidDiscriminator(id1, null!, id3, null!), new GuidDiscriminator(id1, null!, id3, null!)),
	...
	public void Compare_StringDiscriminator()
	{
		...
		Assert.True(comparer.Equals(new StringDiscriminator(id1, id3, null, null), new StringDiscriminator(id1, id3, null, null)),
			"The comparison should pass if identifications(Id) and type identifications(TypeId) are same");
	}
	[Fact(DisplayName = "Discriminator - Validate guid type")]
	public void Validate_GuidDiscriminator()
	{
		var ttt = new GuidDiscriminator(default, "valid", Guid.NewGuid(), "valid");

		// Invalid by Id
		Assert.True(new GuidDiscriminator(default, "valid", Guid.NewGuid(), "valid").IsValid());
