...
public class ZipPod2Test : BaseTest<ZipPod2Test>
{
	...
	public void Zip()
	{
		PayloadBase2 pay = new PayloadDerived2 {
			Name = "payloadName", Age = 23, Nick = "payloadNick"
		};
		var first = pay.BuildPod2("pay");
		Output.WriteLine($"first:\r\n{first.Pod.ToJson()}");
		var json1 = first.ToJson();
		json1.Pod.Headers.Add("header1Payload".BuildPod2("header1").ToJson().Pod);
		json1.Pod.Headers.Add("header2Payload".BuildPod2("header2").ToJson().Pod);
