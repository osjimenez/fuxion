...
public class EventStoreStorage : IEventStorage, ISnapshotStorage
{
	...
	public async Task<IQueryable<Event>> GetEventsAsync(Guid aggregateId, int start, int count)
	{
		...
		var res = client.ReadStreamAsync(Direction.Forwards, aggregateId.ToString(), (ulong)start, count, false);
	...
	{
		var res = client.ReadStreamAsync(Direction.Backwards, aggregateId.ToString(), StreamPosition.End, 1, false);
		var state = await res.ReadState;
		if (state == ReadState.Ok)
		{
			var stream = await res.ToListAsync();
	...
	{
		var res = client.ReadStreamAsync(Direction.Backwards, $"{snapshotType.GetTypeKey()}@{aggregateId.ToString()}", StreamPosition.End, 1, false);
		var state = await res.ReadState;
		if (state == ReadState.Ok)
		{
			var stream = await res.ToListAsync();
