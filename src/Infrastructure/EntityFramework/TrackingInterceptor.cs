		...
		if (interceptionContext.Result.DataSpace == DataSpace.SSpace && interceptionContext.DbContexts.All(con => con.GetType().GetCustomAttribute<TrackDisabledAttribute>() == null))
		{
			...
			if (deleteCommand != null)
			{
				...
				if (bitColumn != null)
				{
					...
					if (dateColumn != null)
						clauses.Add(DbExpressionBuilder.SetClause(deleteCommand.Target.Variable.Property(dateColumn),
							DbExpression.FromDateTimeOffset(TimeZoneInfo.ConvertTime(timeProvider.NowOffsetted().Add(_dateTimeAdjustment), getCurrentTimeZoneFunction()))));
					if (identityColumn != null) clauses.Add(DbExpressionBuilder.SetClause(deleteCommand.Target.Variable.Property(identityColumn), DbExpression.FromString(getCurrentIdentityFunction())));
				}
				if (clauses.Count > 0)
				{
					...
					interceptionContext.Result = new DbUpdateCommandTree(
						deleteCommand.MetadataWorkspace, deleteCommand.DataSpace, deleteCommand.Target, deleteCommand.Predicate, clauses.AsReadOnly(), null);
				}
			}
			#endregion
			...
			if (insertCommand != null)
			{
				...
				if (dateColumn != null)
					AddOrReplaceClause(clauses,
						DbExpressionBuilder.SetClause(insertCommand.Target.Variable.Property(dateColumn),
							DbExpression.FromDateTimeOffset(TimeZoneInfo.ConvertTime(timeProvider.NowOffsetted().Add(_dateTimeAdjustment), getCurrentTimeZoneFunction()))));
				...
				if (dateColumn != null || identityColumn != null)
					interceptionContext.Result = new DbInsertCommandTree(insertCommand.MetadataWorkspace, insertCommand.DataSpace, insertCommand.Target, clauses.AsReadOnly(), null);
			}
			#endregion
			...
			if (updateCommand != null)
			{
				...
				else
				{
					...
					if (dateColumn != null)
						AddOrReplaceClause(clauses,
							DbExpressionBuilder.SetClause(updateCommand.Target.Variable.Property(dateColumn),
								DbExpression.FromDateTimeOffset(TimeZoneInfo.ConvertTime(timeProvider.NowOffsetted().Add(_dateTimeAdjustment), getCurrentTimeZoneFunction()))));
					if (identityColumn != null)
				...
				if (dateColumn != null || identityColumn != null)
					interceptionContext.Result = new DbUpdateCommandTree(
						updateCommand.MetadataWorkspace, updateCommand.DataSpace, updateCommand.Target, updateCommand.Predicate, clauses.AsReadOnly(), null);
			}
			#endregion
		}
	}
	public static void SetDateTimeAdjustment(TimeSpan dateTimeAdjustment) => _dateTimeAdjustment = dateTimeAdjustment;
	void AddOrReplaceClause(ICollection<DbModificationClause> clauses, DbSetClause clause)
	{
		...
		if (res != null) clauses.Remove(res);
		clauses.Add(clause);
	}
	bool ExistClause(IEnumerable<DbModificationClause> clauses, string propertyName)
	{
		var res = clauses.FirstOrDefault(c => ((DbPropertyExpression)((DbSetClause)c).Property).Property.Name == propertyName);
		return res != null;
	}
	void RemoveClause(List<DbModificationClause> clauses, string? propertyName)
	{
		var res = clauses.FirstOrDefault(c => ((DbPropertyExpression)((DbSetClause)c).Property).Property.Name == propertyName);
		if (res != null) clauses.Remove(res);
	}
