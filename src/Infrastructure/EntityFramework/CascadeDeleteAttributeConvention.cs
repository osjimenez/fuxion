...
public class CascadeDeleteConvention : IConceptualModelConvention<AssociationType>
{
	static CascadeDeleteConvention()
	{
		...
		if (associationTypeExtensionsType == null) throw new InvalidProgramException("Assembly 'System.Data.Entity.ModelConfiguration.Edm.AssociationTypeExtensions' cannot be loaded");
	...
	static AssociationEndMember? GetTargetEnd(AssociationType item) => item.KeyMembers.ElementAtOrDefault(1) as AssociationEndMember;
	static CascadeDeleteAttribute? GetCascadeDeleteAttribute(EdmMember edmMember)
	{
		var clrProperties = edmMember.MetadataProperties.FirstOrDefault(m => m.Name == "ClrPropertyInfo");
		if (clrProperties == null) return null;
