...
public static class RabbitDIExtensions
{
		...
		int queueRetryCount = 5)
	{
		...
		builder = sp => {
			var bus = new RabbitMQEventBus(sp, sp.GetRequiredService<IRabbitMQPersistentConnection>(), sp.GetRequiredService<TypeKeyDirectory>(), exchangeName, queueName, queueRetryCount);
